...
from EM_testing.ExtractCustomFields.db_connection import connect_db
sys.path.insert(0,'../..')
...
def Nomination_Insertion(Nom_header_data, Nom_parcel_item, Nom_line_item,error_dict):  
    ...
    try:
            ...
            for table_data in Nom_parcel_item:
                for row in table_data:
                    ...
                    values_2 = (row.get('nom_hd_uuid', ''), row.get('ActivityType', ''), row.get('VendorName', ''),  row.get('ProductName', ''), row.get('NominatedQuantity', ''), row.get(
                        'UnitOfMeasure', ''), row.get('JobLocation', ''), row.get('JobDate', ''), row.get('EmAffiliates', ''), row.get('NominationKey', ''), row.get('VesselName', ''), row.get('status', 0))
                    # print(insert_statement_2,"Nomination Parcel item query", values_2)
        ...
        try:
            ...
            params = UUID
            print("TOSCanaaannanananananananana")
...
def cgi_invoice_db_insertion(inv_header_data, inv_line_item,error_dict):
    ...
    try:
        col_li = ['inv_hd_uuid', 'status',
                  'Description', 'QuantityValue', 'Price']
