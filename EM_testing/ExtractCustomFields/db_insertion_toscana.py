...
def toscana_inv_insertion(final_inv_header_dict,final_inv_qnt,final_inv_qlty):
    ...
    try:
        temp2 = f"SELECT NominationQuantityId from NominationQuantity_External WHERE NominationDetailId={NominationDeatilID}"
        print(temp2)
        cursor.execute(temp2)
        row1 = cursor.fetchone()
        print(row1)
        if row1:
            inspection = row1[0]
            list_of_values.append(inspection)
            list_of_col_heading.append('Inspection')
    except:
        print('error in Inspection')
    GICid=''
    try:
        vendorid=''
        locationid=''
