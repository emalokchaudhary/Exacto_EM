...
def toscana_inv_insertion(final_inv_header_dict,final_inv_qnt,final_inv_qlty):
    ...
    try:
        #temp changes
        act=final_inv_header_dict.get('ActivityType',"")[len(final_inv_header_dict.get('ActivityType',""))-7:]
        # print(act,"#########$")
...
def api_db_toscana_inv_main_call(final_dict_hd, inv_lineitem_dict):
    conn = connect_db()
    cursor = conn.cursor()
    final_dict_hd = inv_header_dict_formatting(final_dict_hd)
...
def api_tos_nomination_main(global_nom_hd):
    ...
    try:
        ...
        # params = 
        Nom_id=global_nom_hd['nom_hd_uuid']
        # print("Toscana data insert query move to exacto",stored_proc,global_nom_hd['nom_hd_uuid'])
        # print("{call HCL_Nomination_Data_Trans('"+global_nom_hd['nom_hd_uuid']+"')}")
        cursor.execute("{call HCL_Nomination_Data_Trans('"+Nom_id+"')}")
        
        # cursor.execute(f"EXEC HCL_Nomination_Data_Trans'{global_nom_hd['nom_hd_uuid']}'")
    except:
        traceback.print_exc()
