...
def Error_update(error_dict):
    try:
        conn = connect_db()
        cursor = conn.cursor()
        insert_error_statement = f"UPDATE exacto_doc_process_log SET ExtractionStatus=5 where doc_uuid='{error_dict['doc_uuid']}'"
        print(insert_error_statement,"FLAGGGGGGGGGGGGGGGGG UPDATEEEEEEEEEEEEEEE")
        cursor.execute(insert_error_statement)
    except Exception as e:
        traceback.print_exc()
        print("Error inserrtion part when insert error dict ")
        pass
...
def toscana_Error_update(docuuid,st):
    try:
        conn = connect_db()
        cursor = conn.cursor()
        insert_error_statement = f"UPDATE exacto_doc_process_log SET ExtractionStatus={st} where doc_uuid='{docuuid}'"
        print(insert_error_statement,"FLAGGGGGGGGGGGGGGGGG UPDATEEEEEEEEEEEEEEE")
        cursor.execute(insert_error_statement)
    except Exception as e:
        traceback.print_exc()
