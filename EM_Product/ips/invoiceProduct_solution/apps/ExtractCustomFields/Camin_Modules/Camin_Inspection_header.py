...
def camin_quantity_header(data,text_data,uuid_doc):
    ...
    logger.info(f'Camin Quantity headerr data -{data_dict}')
    return data_dict
...
def camin_quality_header(data,text_data,uuid_doc):
    ...
    for k,line in enumerate(data):
        ...
        if match:
            ...
            try:
                file_no=re.findall(r'File N° (.*)',text_data)[0].split(":")[1].split("    ")[0].strip()
                # print(file_no)
            except:
                pass
            try: 
                bill_to=" ".join(re.findall(r'(.*)\n(.*)\n(.*)\n(.*)\n(.*)',text_data)[0]).split('Attention')[0].replace("  ","")
                if re.search('certificate of analysis',bill_to,re.I):
                    bill_to=''
                # print(bill_to)
            except:
                pass
    ...
    if bill_to:
        data_dict['EmAffiliates']=bill_to
    data_dict['doc_uuid']=uuid_doc
    logger.info(f'Camin Quality header data -{data_dict}')
    return data_dict
...
# files=os.listdir(r"/datadrive/EM_testing/ExtractCustomFields/Camin_Quantity_document/CHEM-US")
# path=r"/datadrive/EM_testing/ExtractCustomFields/Camin_Quantity_document/CHEM-US"
# for file in files:
