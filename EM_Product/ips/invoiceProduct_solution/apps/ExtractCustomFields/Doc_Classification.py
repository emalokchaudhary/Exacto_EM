...
def Saybolt_classifcation(text_data):
    ...
    try: 
        if re.search('TRIP:', line[0], re.IGNORECASE):
        ...
        elif re.search(r'(Certificate of Quantity)', data, re.IGNORECASE) or re.search(r'(Summary Loading \(GSV\))', data, re.IGNORECASE) or re.search(r'(Loading Summary Report)', data, re.IGNORECASE):
            document_type="Quantity"  
            #print("THIS IS INSPECTION DOCUMENT FOR QUALITY ONLY")
            ...
            document_type="Quality"  
            #print("THIS IS INSPECTION DOCUMENT FOR QUALITY ONLY")
        elif re.search(r'\s+SUMMARY\s+REPORT',re.sub('\s+',' ',data) ,re.I) :        
            data=data.split("\n")
            for i in data:
                if re.search(r'^\s+SUMMARY\s+REPORT',re.sub('\s+',' ',i) ,re.I) :
                    document_type="Quantity"
                    break
                else:
                    document_type="Not classified"
